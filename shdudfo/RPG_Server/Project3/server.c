...
int main(int argc, char* argv[]) {
    ...
    while (1) {
		...
		//clientSocks[clientCount++] = user;//클라이언트 소켓배열에 방금 가져온 소켓 주소를 전달
...
//////////////////////////////////////////영래쿤/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        case MOVE:
            ...
            for (j = 0; j < clientCount; j++)//클라이언트 개수만큼
                send(clientSocks[j].socket, (char*)&sendPoint, sizeof(sendPoint), 0);//클라이언트들에게 메시지를 전달한다.
            ReleaseMutex(hMutex);//뮤텍스 중지
            break;
        case STOPATTACK:
            ...
            WaitForSingleObject(hMutex, INFINITE);//뮤텍스 실행
            for (j = 0; j < clientCount; j++)//클라이언트 개수만큼
                send(clientSocks[j].socket, (char*)&stopAttack, sizeof(stopAttack), 0);//클라이언트들에게 메시지를 전달한다.
            ReleaseMutex(hMutex);//뮤텍스 중지
        ...
        case ATTACK:
            ...
            WaitForSingleObject(hMutex, INFINITE);//뮤텍스 실행
            for (j = 0; j < clientCount; j++)//클라이언트 개수만큼
                send(clientSocks[j].socket, (char*)&sendAttack, sizeof(sendAttack), 0);//클라이언트들에게 메시지를 전달한다.
            ReleaseMutex(hMutex);//뮤텍스 중지
            break;
        case WEAPON:
            ...
            WaitForSingleObject(hMutex, INFINITE);//뮤텍스 실행
            for (j = 0; j < clientCount; j++)//클라이언트 개수만큼
                send(clientSocks[j].socket, (char*)&sendWeapon, sizeof(sendWeapon), 0);//클라이언트들에게 메시지를 전달한다.
            ReleaseMutex(hMutex);//뮤텍스 중지
            break;
        case DAMAGE:
            ...
            WaitForSingleObject(hMutex, INFINITE);//뮤텍스 실행
            for (j = 0; j < clientCount; j++)//클라이언트 개수만큼
                send(clientSocks[j].socket, (char*)&sendDamage, sizeof(sendDamage), 0);//클라이언트들에게 메시지를 전달한다.
            ReleaseMutex(hMutex);//뮤텍스 중지
            break;
    ...
    for (i = 0; i < clientCount; i++) {//배열의 갯수만큼
        if (clientSock == clientSocks[i].socket) {//만약 현재 clientSock값이 배열의 값과 같다면
